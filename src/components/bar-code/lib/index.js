...
function getBuffer({ pieces, width, extraBytes, scale = 1 }) {
  return pieces.map(piece => Array(scale).fill(piece)).reduce((acc, x) => acc.concat(x), []).map((piece, index) => {
  ...
  }).reduce((acc, x) => acc.concat(x), []).map(x => String.fromCharCode(x)).join('');
}
